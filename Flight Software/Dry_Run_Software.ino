...
// ================= ESP8266 WIFI ================
#define ESP Serial3
String laptopIP1 = "192.168.1.101";
String laptopIP2 = "192.168.1.100";
int laptopPort = 5001;
bool wifiOK = false;
bool tcpOK  = false;
...
int para_neutral = 100; // Servo neutral position for parachute servos
float tvc_neutral_position = 1480; // Servo neutral position for TVC mount (PWM)

float altitude_filtered, vel_filtered, yaw_rate_filtered, pitch_rate_filtered;
...
void scanWiFi() {
  ...
  while (millis() - start < 5000) {
    while (ESP.available()) {
      char c = ESP.read();   // read one byte from ESP
      line += c;
...
void connectTCP(String laptopIP) {
  tcpOK = false;
...
void sendTCPfast(String data) { // Send information to laptop
  ...
  // adaptive prompt wait — DO NOT TOUCH
  unsigned long start = millis();
...
void setup() {
  ...
  tvc1.writeMicroseconds(tvc_neutral_position);
  ...
  Serial.println("Configuring ESP...");
  ...
  delay(5000);
  connectWiFi();
  connectTCP(laptopIP1);
  if (!tcpOK) {
    connectTCP(laptopIP2);
  }
  // ---------------- BNO055 INIT ----------------
  if (!bno.begin()) {
  ...
  simFile = SD.open("DRYRUN2.CSV", FILE_READ);
  ...
  PIDInit(controller_pitch, 0.96, 0.12);
  PIDInit(controller_yaw, 0.96, 0.12);
...
// ================= LOOP FUNCTION ================
void loop() {
  float timesim, accelsim, velsim, altsim;
  // Read simulated data
  if (simFile.available()) {
    ...
    timesim = timestr.toFloat()*1000;
    accelsim = accelstr.toFloat();
    velsim = velstr.toFloat();
    altsim = altstr.toFloat();
  }
  else {
  ...
  // ================= STATE DETECTION ================
  static bool motor1Triggered = false;
  static bool motor2Triggered = false;

  // Motor 1 window
  if (timesim >= 5000 && timesim < 6200) {
      digitalWrite(redPin, HIGH);

      if (!motor1Triggered) {
          sendTCPfast("Motor 1 Ignited");
          motor1Triggered = true;
      }
  }
  else {
  ...
  if (timesim >= 13500 && timesim < 16000) {
      digitalWrite(greenPin, HIGH);

      if (!motor2Triggered) {
          sendTCPfast("Motor 2 Ignited");
          motor2Triggered = true;
      }
  }
  else {
  ...
  if (accelsim > 25 && flightPhase == PRELAUNCH) { // Threshold for detection (20)
    if (launch_time == 0) launch_time = last_timesim; // Record first detection
    if (timesim - launch_time > 150) { // Difference between system start and first detection
  ...
  if (accelsim < 25 && flightPhase == LAUNCH) { // Threshold and conditions for detection (20)
    if (burnout_time == 0) burnout_time = last_timesim; // Record first detection
    if (timesim - burnout_time > 300) { // Difference between system start and first detection
  ...
  // Landing
  if ((flightPhase == DESCENT) && (motor2Triggered == true)) { // Condition for detection
  // if (false) {
    values[count] = altsim; // Add samples to array
  ...
  // ================= CSV AND DATA ==========================
  String rowsim = // CSV row
    String(timesim) + "," + String(altsim) + "," + String(velsim) + "," + String(accelsim) + "," + Communication;
  logFile.println(rowsim); // Apend row
  logFile.flush(); // Write row
  Serial.print("Simulated Time: " + String(timesim)) + ",";
  Serial.print("Simulated Altitude: " + String(altsim)) + ",";
  Serial.print("Simulated Velocity: " + String(velsim)) + ",";
  Serial.println("Simulated Acceleration: " + String(accelsim)) + ",";
  last_timesim = timesim;
  last_altsim = altsim;
  last_velsim = velsim;
